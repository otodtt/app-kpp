...
@section('content')
    <div class="info-wrap">
        ...
        <h4 class="bold title_doc" >УДОСТОВЕРЕНИЕ АПТЕКА</h4>
        <hr class="my_hr"/>
        <fieldset class="big_field ">
            <div class="row-height-my col-md-12" style="display: table">
                <div style="display: table-row">
                    ...
                    <div class="small_field_right top_info" style="display: table-cell" >
                        <span class="span-phar-info"><i class="fa fa-plus-square "></i> ДАННИ НА АПТЕКАТА</span>
                    </div>
                </div>
                ...
                <div class="small_field_right" style="display: table-cell">
                    @include('old.objects.documents_all.documents_pharmacy.pharmacy_info')
                </div>
            </div>
            <hr class="my_hr_in"/>
            <div class="col-md-12 row-table-bottom " style="display: table">
                @if($pharmacy->locks == 0)
                    <div class="small_field_bottom" style="display: table-cell">
                        @if(isset($edition) && $edition >0)
                            <a href="{!!URL::to('/редактиране-издание/'.$pharmacy->firm_id.'/'.$pharmacy->id)!!}">
                                <i class="fa fa-edit"></i> Редактирай Издание на Удостоверение</a>
                        @else
                            <a href="{!!URL::to('/аптека/'.$pharmacy->firm_id.'/редактирай/'.$pharmacy->id)!!}">
                    ...
                    <div class="small_field_bottom" style="display: table-cell">
                        {!! Form::model($pharmacy, ['url'=>'locks-pharmacy/'.$pharmacy->id , 'method'=>'POST', 'id'=>'form']) !!}
                ...
                @else
                    ...
                    @if(Auth::user()->admin == 2 )
                        <div class="small_field_bottom" style="display: table-cell">
                            {!! Form::model($pharmacy, ['url'=>'unlocks-pharmacy/'.$pharmacy->id , 'method'=>'POST', 'id'=>'form']) !!}
