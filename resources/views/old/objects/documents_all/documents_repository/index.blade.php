...
@section('content')
    <div class="info-wrap">
        ...
        <h4 class="bold title_doc" >УДОСТОВЕРЕНИЕ СКЛАД</h4>
        <hr class="my_hr"/>
        <fieldset class="big_field ">
            <div class="row-height-my col-md-12" style="display: table">
                <div style="display: table-row">
                    ...
                    <div class="small_field_right top_info" style="display: table-cell" >
                        <span class="span-phar-info"><i class="fa fa-shield"></i> ДАННИ НА СКЛАДА</span>
                    </div>
                </div>

                <div class="small_field_left" style="display: table-cell">
                    @include('old.objects.forms.firm_info_objects')
                </div>
                <div class="small_field_right" style="display: table-cell">
                    @include('old.objects.documents_all.documents_repository.repository_info')
                </div>
            </div>
            <hr class="my_hr_in"/>
            <div class="col-md-12 row-table-bottom " style="display: table">
                @if($repository->locks == 0)
                    <div class="small_field_bottom" style="display: table-cell">
                        @if(isset($edition) && $edition >0)
                            <a href="{!!URL::to('/склад/редактиране-издание/'.$repository->firm_id.'/'.$repository->id)!!}">
                                <i class="fa fa-edit"></i> Редактирай Издание на Удостоверение</a>
                        @else
                            <a href="{!!URL::to('/склад/'.$repository->firm_id.'/редактирай/'.$repository->id)!!}">
                    ...
                    <div class="small_field_bottom" style="display: table-cell">
                        {!! Form::model($repository, ['url'=>'locks-repository/'.$repository->id , 'method'=>'POST', 'id'=>'form']) !!}
                ...
                @else
                    ...
                    @if(Auth::user()->admin == 2 )
                        <div class="small_field_bottom" style="display: table-cell">
                            {!! Form::model($repository, ['url'=>'unlocks-repository/'.$repository->id , 'method'=>'POST', 'id'=>'form']) !!}
                ...
                <div class="col-md-12" id="flip_in">
                    @if(isset($edition) && $edition >0)
                        <div class="col-md-12_my" style="margin: 0 auto">
                            @include('old.objects.documents_all.logo_edition')
                        </div>
                        @include('old.objects.documents_all.edition_body')
                    @else
                        <div class="col-md-12_my" style="margin: 0 auto">
                            @include('old.objects.documents_all.logo')
                        </div>
                        @include('old.objects.documents_all.document_body')
                    @endif
                </div>
            </div>
        </div>
